...
# x is an existing ggplot object. at a minimum, it's a scatter plot of
# the bottom bracket shell center. but it can be a bicycle frame of 
...
What if you had a frame with a classic horizontal top tube, and another with a slant top tube but a shorter seat tube? And what if the chain stay lengths and fork rakes were different too? Or the seat tube angle? Here's a way to compare them:

```{r design_two_frames}
df_slantsix <- wrap_frame_dims(st_length = 460, 
                               cs_length = 430,
                               tt_angle = 6, 
                               st_dt_angle = 58, 
                               cs_st_angle = 62) %>% 
...
df_classic <- wrap_frame_dims(st_dt_angle = 60, 
                              cs_st_angle = 60, 
                              st_angle = 68) %>% 
...
df_slantsix %>% 
    draw_the_bicycle(wheel_diameter = wd, 
                     alpha_factor = al_ss, 
                     i = i, 
                     c = c_ss) %>% 
    ...
    add_the_wheels(df_slantsix, 
                   ...
                   i = i, 
                   c = c_ss) %>% 
    overlay_the_bicycle(df_classic, 
                        wheel_diameter = wd, 
                        alpha_factor = al_ss, 
                        i = i, 
                        c = c_cl) %>% 
    add_the_steering_axis_to_the_drawing(df_classic, 
                                         wheel_diameter = wd, 
                                         alpha_factor = al_ss, 
                                         i = i, 
                                         c = c_cl) %>% 
    add_the_fork_rake_to_the_drawing(df_classic, 
                                     wheel_diameter = wd, 
                                     alpha_factor = al_ss, 
                                     i = i, 
                                     c = c_cl) %>% 
    draw_the_true_fork(df_classic, 
                       wheel_diameter = wd, 
                       alpha_factor = al_ss, 
                       i = i, 
                       c = c_cl) %>% 
    add_the_fork_trail(df_classic, 
                       wheel_diameter = wd, 
                       alpha_factor = al_ss, 
                       i = i, 
                       c = c_cl) %>% 
    add_the_wheels(df_classic, 
                   bb_shell_diameter = 34.8, 
                   wheel_diameter = wd, 
                   alpha_factor = al_ss, 
                   i = i, 
